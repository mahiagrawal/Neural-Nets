...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    out, cache_inside = affine_forward(X, self.params['W1'], self.params['b1'])
    out, cache = relu_forward(out)
    scores, cache_final = affine_forward(out, self.params['W2'], self.params['b2'])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dx, dw, db = affine_backward(dx, cache_final)
    dw += self.reg * self.params['W2']
    grads['W2'] = dw
    grads['b2'] = db

    dx = relu_backward(dx, cache)
    dx, dw1, db1 = affine_backward(dx, cache_inside)
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    cache_relu = {}
    cache_batch = {}
    out[1], cache[1] = affine_forward(X, self.params['W1'], self.params['b1'])
    if self.use_batchnorm:
      out[1], cache_batch[1] = batchnorm_forward(out[1],
                                self.params['gamma1'], 
                                self.params['beta1'], 
                                self.bn_params[0])
    out[1], cache_relu[1] = relu_forward(out[1])

    for i in range(2, self.num_layers):
      out[i], cache[i] = affine_forward(out[i-1], self.params['W'+str(i)], self.params['b'+str(i)])
      if self.use_batchnorm:
        out[i], cache_batch[i] = batchnorm_forward(out[i],
                                self.params['gamma%d'%i], 
                                self.params['beta%d'%i], 
                                self.bn_params[i-1])
      out[i], cache_relu[i] = relu_forward(out[i])
    i = self.num_layers-1
    scores, cache_final = affine_forward(out[i], self.params['W'+str(i+1)], self.params['b'+str(i+1)])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    for i in xrange(self.num_layers-1, 0, -1):
      ...
      dx = relu_backward(dx, cache_relu[i])

      if self.use_batchnorm:
        dx, grads['gamma' + str(i)], grads['beta' + str(i)] = batchnorm_backward(dx, cache_batch[i])
